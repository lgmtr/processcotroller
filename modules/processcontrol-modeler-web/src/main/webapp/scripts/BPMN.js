"use strict";
/*
 * Copyright (C) 1998-2015 by Northwoods Software Corporation. All Rights
 * Reserved.
 */
...
function init() {
	...
	var gradYellow = $(go.Brush, "Linear", {
		0 : "LightGoldenRodYellow ",
		1 : "#FFFF66"
	});
	var paper = $(go.Brush, "Linear", {
		0 : "OldLace",
		1 : "PapayaWhip"
	});
	var gradLightGreen = $(go.Brush, "Linear", {
		0 : "#E0FEE0",
		1 : "PaleGreen"
	});
	var gradLightGray = $(go.Brush, "Linear", {
		0 : "White",
		1 : "#DADADA"
	});
	...
	var handGeo = go.Geometry
			.parse("F1M18.13,10.06 C18.18,10.07 18.22,10.07 18.26,10.08 18.91,"
	...
	var tooltiptemplate = $(go.Adornment, go.Panel.Auto, $(go.Shape,
			"RoundedRectangle", {
				fill : "whitesmoke",
				stroke : "gray"
			}), $(go.TextBlock, {
	...
	function nodeEventTypeConverter(s) { // order here from BPMN 2.0 poster
		var tasks = [ "NotAllowed", "Empty", "BpmnTaskMessage",
				"BpmnEventTimer", "BpmnEventEscalation",
				"BpmnEventConditional", "Arrow", "BpmnEventError", "ThinX",
				"BpmnActivityCompensation", "Triangle", "Pentagon",
				"ThickCross", "Circle" ];
		if (s < tasks.length)
			return tasks[s];
		return "NotAllowed"; // error
	...
	// removing a boundary event doesn't not reposition other BE circles on the
	// node
	...
	var boundaryEventItemTemplate = $(go.Panel, "Spot", {
		...
		fromMaxLinks : 1,
		toMaxLinks : 0
	}, new go.Binding("portId", "portId"), new go.Binding("alignment",
			"alignmentIndex", nodeActivityBESpotConverter), $(go.Shape,
			"Circle", {
	...
	function makeMarkerPanel(sub, scale) {
		return $(go.Panel, "Horizontal", {
			...
			strokeWidth : activityMarkerStrokeWidth
		}, new go.Binding("visible", "isLoop")), $(go.Shape,
				"BpmnActivityParallel", {
					...
					visible : false,
					strokeWidth : activityMarkerStrokeWidth
				}, new go.Binding("visible", "isParallel")), $(go.Shape,
				"BpmnActivitySequential", {
					...
					visible : false,
					strokeWidth : activityMarkerStrokeWidth
				}, new go.Binding("visible", "isSequential")), $(go.Shape,
				"BpmnActivityAdHoc", {
					width : 12 / scale,
					height : 12 / scale,
					margin : 2,
					visible : false,
					strokeWidth : activityMarkerStrokeWidth
				}, new go.Binding("visible", "isAdHoc")), $(go.Shape,
				...
				makeSubButton(sub)); // end activity markers horizontal panel
	}
	...
	var activityNodeTemplate = $(go.Node, "Spot", {
		...
		itemTemplate : boundaryEventItemTemplate
			...
			"location", "loc", go.Point.parse).makeTwoWay(go.Point.stringify),
	...
	new go.Binding("layerName", "isSelected", function(s) {
		...
		desiredSize : new go.Size(activityNodeWidth, activityNodeHeight)
			...
			.makeTwoWay(go.Size.stringify), $(go.Panel, "Spot", $(go.Shape,
			...
			}, new go.Binding("fill", "color"), new go.Binding("strokeWidth",
					"isCall", function(s) {
						return s ? activityNodeStrokeWidthIsCall
								: activityNodeStrokeWidth;
					})),
			// $(go.Shape, "RoundedRectangle", // the inner "Transaction"
			// rounded rectangle
			// { margin: 3,
	...
	$(go.TextBlock, // the center text
	{
		...
		margin : 12,
		editable : true
	}, new go.Binding("text").makeTwoWay()))); // end go.Node

	// ---------------------------------------- template for Activity / Task
	// node in Palette
	...
	var activityNodeTemplateForPalette = $(go.Node, "Vertical", {
		...
		name : "PANEL",
		desiredSize : new go.Size(activityNodeWidth / palscale,
	...
	}), new go.Binding("stroke", "eventDimension",
			nodeEventDimensionStrokeColorConverter), new go.Binding(
			"strokeDashArray", "eventDimension", function(s) {
				return (s === 3 || s === 6) ? [ 4, 2 ] : null;
			}), new go.Binding("desiredSize", "size", go.Size.parse)
			.makeTwoWay(go.Size.stringify)), // end main shape
	...
	}), new go.Binding("desiredSize", "gatewayType",
	...
	$(go.Shape, "Circle", // Inner circle
	{
		alignment : go.Spot.Center,
		stroke : gatewayNodeSymbolStroke,
		desiredSize : new go.Size(eventNodeInnerSize / 2,
	...
	var dataObjectNodeTemplate = $(go.Node, "Vertical", {
		...
		locationSpot : go.Spot.Center
			...
			.makeTwoWay(go.Point.stringify), $(go.Shape, "File", {
		name : "SHAPE",
		portId : "",
		fromLinkable : true,
		toLinkable : true,
	...
	var dataStoreNodeTemplate = $(go.Node, "Vertical", {
		...
		locationSpot : go.Spot.Center
			...
			.makeTwoWay(go.Point.stringify), $(go.Shape, "Database", {
		name : "SHAPE",
		portId : "",
		fromLinkable : true,
		toLinkable : true,
	...
	}), $(go.Panel, "Table", // table with 2 cells to hold header and lane
	{
		...
		minSize : new go.Size(activityNodeWidth, activityNodeHeight * 0.667)
			...
			.makeTwoWay(go.Size.stringify), $(go.TextBlock, {
		row : 0,
		column : 0,
		angle : 270,
		margin : 5,
	...
	$(go.Panel, "Auto", $(go.Shape, "RoundedRectangle", {
		...
		toSpot : go.Spot.LeftSide
			...
			.makeTwoWay(go.Size.stringify), new go.Binding("strokeWidth",
			"isCall", function(s) {
				return s ? activityNodeStrokeWidthIsCall
						: activityNodeStrokeWidth;
			})), $(go.Panel, "Vertical", {
	...
	}, $(go.TextBlock, // label
	{
		margin : 3,
		editable : true
	}, new go.Binding("text", "text").makeTwoWay(), new go.Binding("alignment",
			"isSubGraphExpanded", function(s) {
	...
	var sequenceLinkTemplate = $(go.Link, {
		...
		visible : false
	}, new go.Binding("text", "text").makeTwoWay(), new go.Binding("visible",
			"visible").makeTwoWay()));
	...
	// uncomment this if you want a subprocess to expand on drop. We decided we
	...
	// this is called after nodes have been moved or lanes resized, to layout
	// all of the Pool Groups again
	...
	LaneResizingTool.prototype.canStart = function() {
		...
		if (diagram === null)
			return;
		var handl = this.findToolHandleAt(diagram.firstInput.documentPoint,
				this.name);
	...
	// define a custom grid layout that makes sure the length of each lane is
	// the same
	...
	function PoolLayout() {
		...
		// This is useful when Groups can be moved up and down in order to
		// change their order.
			...
			pool.memberParts.each(function(lane) {
				...
				if (lane.category !== "Pool") {
					...
					if (shape !== null) { // change the desiredSize to be big
						...
						shape.height = (!isNaN(shape.height)) ? Math.max(
								shape.height, sz.height) : sz.height;
						var cell = lane.resizeCellSize;
						if (!isNaN(shape.width) && !isNaN(cell.width)
								&& cell.width > 0)
							shape.width = Math.ceil(shape.width / cell.width)
									* cell.width;
						if (!isNaN(shape.height) && !isNaN(cell.height)
								&& cell.height > 0)
							shape.height = Math
									.ceil(shape.height / cell.height)
									* cell.height;
					}
				}
			});
		}
		// now do all of the usual stuff, according to whatever properties have
		// been set on this GridLayout
	...
	// this is a Part.dragComputation function for limiting where a Node may be
	// dragged
	function stayInGroup(part, pt, gridpt) {
		...
		// find the padding inside the group's placeholder that is around the
		// member parts
		var m = grp.placeholder.padding;
		// now limit the location appropriately
		var x = Math.max(p1.x + m.left, Math.min(pt.x, p2.x - m.right - b.width
				- 1))
				+ (loc.x - b.x);
		var y = Math.max(p1.y + m.top, Math.min(pt.y, p2.y - m.bottom
				- b.height - 1))
				+ (loc.y - b.y);
	...
	function addLaneEvent(lane) {
		...
		if (lane != null && lane.data.category === "Lane") {
			...
			var newlanedata = {
				...
				color : "white",
				isGroup : true,
				loc : go.Point.stringify(new go.Point(lane.location.x,
						lane.location.y + 1)), // place below selection
	...
	new go.Binding("fill", "color"), new go.Binding("desiredSize", "size",
	...
	swimLanesGroupTemplate.resizeAdornmentTemplate = $(go.Adornment, "Spot",
			$(go.Placeholder), $(go.Shape, // for changing the length of a lane
			{
	...
	// taskType: 1,
	// item: "Message Task",
	// boundaryEventArray: [{ "portId": "be0", alignmentIndex: 0, eventType: 2,
...
// Add a port to the specified side of the selected nodes. name is beN (be0,
// be1)
			...
			.each(function(node) {
				...
				if (node.data
					...
					var newportdata = {
						...
						color : "white",
						alignmentIndex : i
					// if you add port data properties here, you should copy
					// them in copyPortData above ** BUG... we don't do that.
					};
					// and add it to the Array of port data
					myDiagram.model.insertArrayItem(
							node.data.boundaryEventArray, -1, newportdata);
				}
			});
...
function newDocument() {
	...
	if (myDiagram.isModified) {
		var save = confirm("Would you like to save changes to "
				+ getCurrentFileName() + "?");
		if (save) {
			saveDocument();
		}
